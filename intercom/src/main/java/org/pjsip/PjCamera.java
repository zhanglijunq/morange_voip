...
public class PjCamera implements Camera.PreviewCallback, SurfaceHolder.Callback
{
    ...
    {
    	Log.i(TAG, "SetSurfaceView, sv="+sv);
    ...
    public int Start()
    {
        ...
        Log.i(TAG, "[zzy]camera open OK, idx:"+camIdx);
        ...
        Log.d(TAG, "[zzy]start camera preview");
        isRunning = true;
    ...
    public void Stop()
    {
        ...
        camera.stopPreview();
        Log.d(TAG, "[zzy]stop camera preview");
        camera.release();
        camera = null;
    }
    ...
    {
        //Log.i(TAG, "[zzy]onPreviewFrame");
        if (debugFirstRaw == 0)
        {
            debugFirstRaw = (int) System.currentTimeMillis();
            Log.i(TAG, "[zzy]onPreviewFrame, 1st preview raw data, len="+data.length);
        }
