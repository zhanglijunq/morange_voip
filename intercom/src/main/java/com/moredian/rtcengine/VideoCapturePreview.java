...
// complete.
public class VideoCapturePreview implements PreviewCallback, Callback {
	private final static String TAG = "VideoCapturePreview";
	...
	// private final long native_capturer; // |VideoCapturePreview*| in C++.
...
//	private static List<OnVideoCaptureListener> allListeners= new ArrayList<OnVideoCaptureListener>();
	private static ReentrantLock listenerLock = new ReentrantLock();
	private RTCVideoCaptureListener mCaptureListener = null;
	...
	public static void setLocalPreview(SurfaceHolder localPreview) {
		...
		VideoCapturePreview.localPreview = localPreview;
	}

	public VideoCapturePreview(int id, Context context) {
		this.id = id;
		// this.native_capturer = native_capturer;
		this.context = context;
	}

	public void setCaptureListener(RTCVideoCaptureListener listener){
	...
	public synchronized boolean startCapture(final int width, final int height) {
	...
	private void startCaptureOnCameraThread(int width, int height,
...
//			PjSipService.setUseAppCamera(false);
			if (localPreview != null) {
				...
				try {
					...
					cameraSurfaceTexture = new SurfaceTexture(
							cameraGlTextures[0]);
			...
			Log.d(TAG, "Camera preview mfps range: " + min_mfps + " - "
					+ max_mfps);
...
//			parameters.setPreviewFormat(format);
			...
			int bufSize = width * height * ImageFormat.getBitsPerPixel(format)
					/ 8;
	...
	private void stopCaptureOnCameraThread(Exchanger<Boolean> result) {
		...
		try {
			...
			camera.release();
			camera = null;
//			PjSipService.setUseAppCamera(true);
			exchange(result, true);
	...
	// private native void ProvideCameraFrame(byte[] data, int length,
	// int rotation, long timeStamp, long captureObject);

	// Called on cameraThread so must not "synchronized".
	@Override
	public void onPreviewFrame(byte[] data, Camera callbackCamera) {
		// Log.i(TAG, "enter onPreviewFrame==================");
...
//		Log.i(TAG, "enter onPreviewFrame==================");
		...
		if (frameCount > frameDropRatio) {
			...
			if ((frameCount % 30) == 0) {
				Log.d(TAG, "Camera TS " + captureTimeMs + ". Duration: "
						+ (int) durationMs + " ms. FPS: "
		...
		Parameters param = callbackCamera.getParameters();
		Size siz = param.getPreviewSize();
		if(mCaptureListener!=null)
			mCaptureListener.onFrame(data, data.length, rotation,captureTimeMs);
		camera.addCallbackBuffer(data);
	}
	...
	public synchronized void surfaceChanged(SurfaceHolder holder, int format,
		...
		Log.d(TAG, "VideoCapturePreview::surfaceChanged ignored: " + format
				+ ": " + width + "x" + height);
	}

	@Override
	public synchronized void surfaceCreated(final SurfaceHolder holder) {
		Log.d(TAG, "VideoCapturePreview::surfaceCreated");
		if (camera == null || cameraThreadHandler == null) {
			return;
		}
		final Exchanger<IOException> result = new Exchanger<IOException>();
	...
	public synchronized void surfaceDestroyed(SurfaceHolder holder) {
		Log.d(TAG, "VideoCapturePreview::surfaceDestroyed");
		if (camera == null || cameraThreadHandler == null) {
			return;
		}
		final Exchanger<IOException> result = new Exchanger<IOException>();
	...
	private void setPreviewDisplayOnCameraThread(SurfaceHolder holder,
			Exchanger<IOException> result) {
		try {
